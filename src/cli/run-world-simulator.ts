...
async function main() {
  ...
  const pipelineDefinition =
    pipelineRegistry[pipelineId as keyof typeof pipelineRegistry];
  if (!pipelineDefinition) {
    console.error(chalk.red(`❌ Pipeline with ID "${pipelineId}" not found!`));
    console.error(
      chalk.red(
        `Available pipelines: ${Object.keys(pipelineRegistry).join(", ")}`
      )
    );
    process.exit(1);
  }
  ...
  const cliParams = ((args.params as string[] | undefined) || []).reduce(
    (acc: Record<string, any>, pair: string) => {
  ...
  if (framePath) {
    ...
    pipelineId = world.pipelineId;
    console.log(
      chalk.green(`✅ Forked world created with new runId: ${world.runId}`)
    );
  } else {
    console.log(
  ...
  console.log(
    chalk.blueBright(`🔧 Pipeline Definition: ${pipelineDefinition.label}`)
  );
  ...
  if (!result || typeof result.tick !== "number") {
    console.error(
      chalk.red("⚠️ Simulation failed or returned invalid result.")
    );
    process.exit(1);
  }

  console.log(
    chalk.green(`✅ Simulation completed! Final tick: ${result.tick}`)
  );
  if (result.context && result.context._artifactsById) {
    const archive = (
      Array.from(result.context._artifactsById.values()) as SymbolicObject[]
    ).find((o: SymbolicObject) => o.type === "WorldArchive");
    ...
    if (typedArchive?.filePath) {
      ...
      console.log(
        chalk.gray(`📄 Archive complete, but file path not recorded.`)
      );
    }
  }

...
it('forks a world and retains aonst userArgs: Prtifacts and context', () => {
  ...
  expect(forked.context.userArgs.params.firebreak).toBe(true);
});
...
describe('world-simulator', () => {
  ...
  it('creates a world from frame with correct artifacts', () => {
    const mockFrame: SymbolicWorldFrame = {
      id: 'frame-1',
      tick: 3,
      step: 2,
      runId: 'abc',
      type: 'SymbolicWorldFrame',
      pipelineId: 'mock-pipeline',
      createdAt: new Date().toISOString(),
      status: 'active',
      members: [{ id: 'x', type: 'MockObject', label: 'X', createdAt: new Date().toISOString() }],
    };
...
it('initializes tick, step, and artifacts correctly from frame', () => {
  const mockFrame: SymbolicWorldFrame = {
    ...
    createdAt: new Date().toISOString(),
    members: [
      { id: 'alpha', type: 'TestObject', createdAt: new Date().toISOString() },
      { id: 'beta', type: 'TestObject', createdAt: new Date().toISOString() },
    ],
  };
...
it('forks a world from a frame and retains pipelineId and forkedFromRunId', () => {
  const mockFrame: SymbolicWorldFrame = {
    ...
    createdAt: new Date().toISOString(),
    members: [
      { id: 'a', type: 'Thing', createdAt: new Date().toISOString() },
      { id: 'b', type: 'Thing', createdAt: new Date().toISOString() },
    ],
  };
...
it('runs a different pipeline after forking and generates a distinct PipelineRun', async () => {
  const baseFrame: SymbolicWorldFrame = {
    id: 'base-frame',
    tick: 2,
    step: 2,
    runId: 'base-run',
    ...
    status: 'active',
    createdAt: new Date().toISOString(),
    members: [{ id: 'x', type: 'BaseObject', createdAt: new Date().toISOString() }],
  };
...
it('creates multiple forks from a base world and runs separate pipelines', async () => {
  const baseFrame: SymbolicWorldFrame = {
    ...
    createdAt: new Date().toISOString(),
    members: [
      { id: 'a', type: 'Symbol', createdAt: new Date().toISOString() },
      { id: 'b', type: 'Symbol', createdAt: new Date().toISOString() },
    ],
  };
  ...
  results.forEach((world, i) => {
    const branchResult = world.artifacts.get(`result-fork-${String.fromCharCode(65 + i)}`);
